      ...
      class FunctionGenerator < Retrospec::Plugins::V1::Plugin
        ...
        def initialize(module_path, spec_object = {})
          ...
          @context = OpenStruct.new(:name => spec_object[:name], :return_type => spec_object[:return_type],
                                    :function_type => spec_object[:type].downcase)
        end
        ...
        def template_dir
          unless @template_dir
            ...
            if File.exist?(external_templates)
              @template_dir = File.join(config_data[:template_dir], 'functions', function_type)
            else
              @template_dir = File.expand_path(File.join(File.dirname(File.dirname(__FILE__)), 'templates', 'functions', function_type))
            end
          end
          @template_dir
        end
