...
describe "facter" do
  ...
  it 'can eval code' do
    model = Retrospec::Puppet::Generators::Facter.load_fact(file)
    fact_name = model.keys.first
    confines = model[fact_name].confines
    model_facts = model[fact_name].used_facts
  ...
  describe 'multiple facts' do
    ...
    it 'can eval code with multiple facts' do
      ...
      fact_name = models.keys.first
      fact_data = models[fact_name]
